# March 20th 2020
# change data structure of board from dict to numpy array
# change player numbers / player ids from 0 and 1 to -1 and 1
# change empty spots on board from -1 to 0
# new error: stone set at boundary is not accepted
...
class Player:
    ...
    Players are identified by their (number), either -1 or 1."""
    def __init__(self):
...
class Board:
    ...
    def update(self, draw):
        ...
        for direction in draw.directions_enclosing:
            ...
            while not done:
                next_stone_in_line = pos.step(direction)
    ...
    def put_stone_on_board(self, draw):
        ...
        self.board[draw.position] = draw.player
        self.stones_set += 1
...
class RuleChecker:
    ...
    def select_directions_touching(self, directions):
        ...
        return tuple(filter(lambda x: self.check_for_different_colour(self.draw.position.step(x), self.draw.player), directions))
    ...
    def walk_on_beam(self, colour, position, direction):
        ...
        next_pos = position.step(direction)
    ...
    def select_directions_enclosing(self):
        ...
        return list(filter(lambda d: self.walk_on_beam(player_id, start_pos.step(d), d), dir_touch_opponent))
